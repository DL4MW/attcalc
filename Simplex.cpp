...
CSimplex::CSimplex(int ndim, double tol, int maxiter, double (*func)(std::vector<double>))
{
	m_NoDim = ndim;
	m_Tol = tol;
	m_Func = func;
...
*/
	...
	while(m_NoEval < m_MaxIter)
	{
		...
		if(m_Write)
		{
			cout << setw(4) << count << "   " << setw(WIDTH) << vFx[vEx[2]] << "   ";
					 ...
					 const std::vector<int> vEx, double scale, double& fmax)
{
	...
	{
            for(int j = 0; j < m_NoDim; j++){
                if(vNext[j]<0)vNext[j]=0;  // !!! Änderung, auf 0 beschränken
